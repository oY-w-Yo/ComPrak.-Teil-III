...
def k_closest_point(Point,pSet,k):
    for p in pSet:
        p.update('Point' = Point, 'DistanceToPoint' = distance_sq(p.get('Vector'),Point))
    pSet.sort(key = lambda p: p.get('DistanceToPoint'))
    return pSet[:k]
...
def ErrorCal(func,TestSet):
    m = len(TestSet)
    Error = sum([labeled_point.get('Label') != func(labeled_point.get('Vektor')) for labeled_point in TestSet])
    Error = Error/m
    return Error
