...
def classify_kd (name,KSET,l,Folder,shufflee=True):
    ...
    def f_D_k_result(Point):
        ...
        for i in range(l):
            k_star_best_in_i,distance_set_in_i = copy.deepcopy(tree_root_list_with_i[i].search_k_nearst_from_kd_node(Point,k_star))
            for m in range(len(k_star_best_in_i)):
                k_star_best_in_i[m].append(distance_set_in_i[m]) 
                k_star_best_in_i[m].append(i) 
            #print('kd:k_star_best_in_{}={}'.format(i,k_star_best_in_i))
            k_star_best.extend(k_star_best_in_i)
        #print('kd:k_star_best={}'.format(k_star_best))
