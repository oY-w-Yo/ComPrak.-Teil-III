    ...
    def search_k_nearst_from_ball_node(self,Point,k):
        ...
        while Stack != []:
            ...
            if current_node.position == AC.Position.Leaf:
                ...
                if len(k_Best) < k or distance_set[-1] > AF.distance_max_Ball(Point,current_node.pivot[1],current_node.radius):
                    #Stack.append(current_node)
...
def classify_ball (name,KSET,l,Folder,shufflee=True):
    ...
    def f_D_k_result(Point):
        ...
        for i in range(l):
            k_star_best_in_i,distance_set_in_i = copy.deepcopy(tree_root_list_with_i[i].search_k_nearst_from_ball_node(Point,k_star))
